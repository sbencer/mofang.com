...
<SCRIPT LANGUAGE="JavaScript">
  ...
  if(window.top.$('#<?php echo $field?>').val()) {
	...
	function addMarker(){ 	  
		...
		tipOption.tipType = MConstants.HTML_BUBBLE_TIP;//信息窗口標題  
		tipOption.title = address;//信息窗口標題  
		tipOption.content = address;//信息窗口內容   
...
var mapObj = new BMap.Map("mapObj");          // 創建地圖實例  
//向地圖中添加縮放控件
var ctrl_nav = new BMap.NavigationControl({anchor:BMAP_ANCHOR_TOP_LEFT,type:BMAP_NAVIGATION_CONTROL_LARGE});
mapObj.addControl(ctrl_nav);
mapObj.enableDragging();//啟用地圖拖拽事件，默認啟用(可不寫)
mapObj.enableScrollWheelZoom();//啟用地圖滾輪放大縮小
mapObj.enableDoubleClickZoom();//啟用鼠標雙擊放大，默認啟用(可不寫)
mapObj.enableKeyboard();//啟用鍵盤上下左右鍵移動地圖
