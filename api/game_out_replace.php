...
while($start>=0){
	...
	$rs = $gameout_db->query("select id,gameid,title,package_name,status from www_gameout ORDER BY id desc limit $limit");
	...
	if(!empty($data)){
		foreach ($data as $key => $value) {
			if($value['status']==0){
				//請求李偉接口，傳遞數據
				if($type==1){
					$package_name = urlencode($value['title']);
					$new_status = 5;
				}else{
					$package_name = strtolower($value['package_name']);
					$new_status = 4;
				}
				$request_api = "http://game.mofang.com/api/web/SaveOldGameId?package_name=".$package_name."&old_gameid=".$value['gameid']."&type=".$type;
				...
				if($datas['code']=='0') {
					...
					$gameout_db->update(array("status"=>$new_status),array("id"=>$value['id']));
					$num++;
				}
			}
