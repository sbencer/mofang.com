...
class defense {
	...
	function __construct() {
        ...
        if ($_SERVER['SERVER_NAME'] == 'a.mofang.com') {
            $this->modelid = 21;
            $this->catid = 134;
        } else {
            $this->modelid = 20;
	...
	function banner() {
		...
		$banner = $this->db->select(array('catid'=>'694'),"title,thumb,url",3);
	 ...
	 * 獲得所有遊戲列表
	 */
	protected function gamelist($tag_ids=array(),$page='',$order='',$free=0) {
		// echo 'gamelist';
		// 查詢條件
		if($page != '') {
			$perpage = 10;
		...
		if($free != 0) {
			// 查詢所有免費遊戲
			$result = $this->price_db->select(array('price_number'=>0), 'id');
			foreach ($result as $value) {
				$frees_index[] = $value['id'];
			}
			// 獲得符合篩選條件和免費遊戲的交集
			$games_index = array_intersect($games_index,$frees_index);
		}
		...
		// 從遊戲庫中查詢遊戲信息
		if(!empty($index)) {
            $this->db->query("SELECT i.id,title,icon,icon_tag,price_number,price,price_unit,score,release_time FROM {$this->db->table_name} i LEFT JOIN {$this->db->table_name}_data d ON i.id=d.id WHERE i.id IN ({$index}) {$order} {$limit}");
	...
	function ranks($data) {
		...
		foreach ($ranks as $key => $value) {
			...
			$ranks[$key]['played'] = ($played['views'] + 5 ) * 1234;
			$ranks[$key]['score'] *= 2;
	...
	function free($data) {
		...
		$result1 = $this->db->query("SELECT i.id,icon,title,price,price_number,price_unit,score FROM {$this->db->table_name} i LEFT JOIN {$this->db->table_name}_data d ON d.id=i.id WHERE price_number=0 AND price_number<price");
		$apps_free = $this->db->fetch_array();
		$result2 = $this->db->query("SELECT i.id,icon,title,price,price_number,price_unit,score FROM {$this->db->table_name} i LEFT JOIN {$this->db->table_name}_data d ON d.id=i.id WHERE ((price_number>=6 AND price_unit=1) OR (price_number>=0.99 AND price_unit=2)) AND price_number<price");
