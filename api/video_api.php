...
if(!empty($_GET['do_complete']) && !empty($_GET['uid']) && !empty($_GET['snid'])){
	...
	if(empty($sndaid)){
		showmessage("請通正常路徑訪問網站！",APP_PATH);
	}
	showmessage("請完善注冊信息資料！",APP_PATH.'index.php?m=video&c=video&a=complete_info&uid='.intval($_GET['uid']).'&snid='.$sndaid.'&pc_hash='.htmlspecialchars($pc_hash));
}

if(!empty($_GET['skey']) && !empty($_GET['sn'])){
	header("Location: ".APP_PATH.'index.php?m=video&c=video&a=set_video_setting&skey='.$_GET['skey'].'&sn='.$_GET['sn'].'&pc_hash='.htmlspecialchars($pc_hash));
}
