...
class rss {
	...
	public function get_json_bypartition(){
	    ...
	    foreach ($info as $arr) {
			...
			$new_r = array();
			$new_r['id'] = $r['id'];
			$new_r['catname'] = $cat_array['catname'];
			$new_r['gamename'] = $cat_array['gamename'];
			$new_r['title'] = $r['title'];
			$new_r['thumb'] = $r['thumb'];
			$new_r['description'] = $r['description'];
			$new_r['content'] = $r['content'];
			$new_r['inputtime'] = $r['inputtime'];
			$new_r['author'] = '魔方网台湾站';
			$return_array[] = $new_r;
