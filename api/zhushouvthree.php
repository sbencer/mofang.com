...
if( $_GET['catid'] ){//欄目文章列表
	...
	if( $part_info['child'] ){//父級欄目
	    ...
	    $data = article_lists( intval($_GET['catid']), false, $time_from);
 ...
 *
 **/
function make_category_structure($catid, &$father_partition_list) {

	global $db_partition;
	$time_from = isset($_GET['time_from']) ? intval($_GET['time_from']) : 0;

	$son_list = $db_partition->get_one('`catid`='.$catid, '*', '', 'listorder ASC');

	if(!empty($son_list)){
		if($son_list['child'] == 1){
			$childs = explode(',', $son_list['arrchildid']);
			foreach($childs as $child) {
				if($child != $son_list['catid']) {
					make_category_structure($child, $tmp[]);
				}
			}
		} else {
			$tmp = article_lists($son_list['catid'], true, $time_from);
		}
	}

	if($time_from != 0) {
		$father_partition_list['catid'] = $son_list['catid'];
		$father_partition_list['child'] = $son_list['child'];
	} else {
		unset($son_list['setting']);
		$father_partition_list = $son_list;
	}
	$father_partition_list['childs'] = $tmp;
}
...
function get_partition_structure( $device, $package_name, $time_from=0 ){
	...
	$device_column = ($device=='android') ? "`pack_android`" : "`pack_ios`";
	$father_partition_item = $db_partition->get_one( $device_column."='".$package_name."'" ); 
	if($father_partition_item['setting'] != '') {

        //專區英文標示 
        define('DOMAIN',$father_partition_item['domain_dir']);

		$event_result = array();
		$setting = string2array($father_partition_item['setting']);
		foreach( $setting['button_v2'] as $key=>$value ){
			if($value['button_type'] == 1) {
				$curr_son_list_temp = array();
				make_category_structure($value['button_value'], $curr_son_list_temp);
				$event_result[] = $curr_son_list_temp;
        ...
        //廣告列表
		$partition_list_info['ad_pic'] = array_values($temp_setting['ad_pic']);
			//json_decode($father_partition_item['ad_pic'], true);
...
function article_lists($catid, $return = false, $time_from=0){
	...
	foreach( $part_article_list as $key=>$value ){
        ...
        $temp_field = ($value['modelid']==3) ? 'id,catid,url,title,thumb,updatetime,islink' : 'id,catid,url,title,shortname,thumb,updatetime,islink';
        ...
        if($temp_article_info['islink']) {
            $temp_article_info['url'] = $temp_article_info['url'].'?from=mofang';
        } else {
            $temp_article_info['url'] =  APP_PATH . DOMAIN . '/' . $temp_article_info['catid'] . '_' . $temp_article_info['id'] . '.html';
        }
	...
	if ($return) {
	    return !empty($article_list) ? $article_list : array();
	} else {
