...
/**     hit eggs                         **/
/******************************************/
define('eggsui',['jquery','mfe'],function(require,exports,module){
    ...
    eggsUi.dom = {
        ...
        css : function(id,property,value){
            ...
            if(value){
                if(property=='opacity'){
                    eggsUi.dom.setOpacity(id,value);
                }else{
        ...
        ready : function(f){
            ...
            if (!ie && !wk && d.addEventListener)
                return d.addEventListener('eggsUi.domContentLoaded', f, false);
    ...
    eggsUi.effect = {
        ...
        fade : function(id,startOpacity,stopOpacity,duration){
            ...
            if (startOpacity < stopOpacity){
                var timer = setInterval(function(){//淡入
                    eggsUi.dom.setOpacity(id,++startOpacity);
        ...
        fadeIn : function(id,duration){
            eggsUi.effect.fade(id,0,100,duration);
        },
        /** 淡出 id元素对象；duration持续时间  **/
        fadeOut : function(id,duration){
            eggsUi.effect.fade(id,100,0,duration);
        },
        ...
        mask : function(show) {
            var fbg = eggsUi.dom.ID("__frame__");
            if(!fbg){
                var html;
                html = '<div id="__frame__" style="display:none;eggsUi.position:absolute;top:0;left:0;background:#000;filter:alpha(opacity=60);-moz-opacity:0.6;-khtml-opacity: 0.6;opacity: 0.6;z-index:9999;border:none;width:100%;">';
                ...
                html += '</iframe>';
                html += '</div>';
                eggsUi.dom.insertHtml(html);
            }
            fbg = eggsUi.dom.ID("__frame__");
            if(show){
                fbg.style.height = eggsUi.browser.getDocHeight() + "px";
                fbg.style.display = "";
        ...
        zoomIn : function(id,mask){
            ...
            var timer = setInterval(function(){
                ...
                eggsUi.dom.setOpacity(id,opacity);
                opacity += 10;
                ...
                eggsUi.position.center(id);
                if(height_over&&width_over){
                    clearInterval(timer);
                    if(mask) eggsUi.effect.mask(1);//要等动画结束后再加遮罩。因为至少在IE8里发现有遮罩的话，动画会非常卡。
                }
            },10);
        },
        /** 以快速缩小的方式居中消息 id元素对象;mask是否有加遮罩 **/
        zoomOut : function(id,mask){
            if(mask) eggsUi.effect.mask(0);
            id = eggsUi.dom.ID(id);
    ...
    eggsUi.position = {
        /** 居中 id元素对象 **/
        center : function(id){
             var id = eggsUi.dom.ID(id);
             var ie6 = eggsUi.browser.ie6();
             var win = eggsUi.browser.getWindow();//浏览器窗口宽度和高度
             var ele = {'height':id.clientHeight,'width':id.clientWidth};//元素对象的宽度和高度
             if(ie6){
                 id.style.position = 'absolute';
                 var scrollBar = eggsUi.browser.getScroll(); //滚动条宽度和高度
             }else{
        ...
        floatCenter : function(id){
            eggsUi.position.center(id);
            var fun = function(){eggsUi.position.center(id);};
            if(eggsUi.browser.ie6()){
                eggsUi.myEvent.add(window,'scroll',fun);
                eggsUi.myEvent.add(window,'resize',fun);
            }else{
                eggsUi.myEvent.add(window,'resize',fun);
            }
        },
        ...
        floatRightBottom : function(id){
            eggsUi.position.rightBottom(id);
            var fun = function(){eggsUi.position.rightBottom(id);};
            if(eggsUi.browser.ie6()){
                eggsUi.myEvent.add(window,'scroll',fun);
                eggsUi.myEvent.add(window,'resize',fun);
            }
