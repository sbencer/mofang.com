function att_show(serverData,file)
{
	...
	if(ext == 1) {
		var img = '<a href="javascript:;" onclick="javascript:att_cancel(this,'+id+',\'upload\')"><div class="icon"></div><img src="'+src+'" width="80" imgid="'+id+'" path="'+src+'" title="'+filename+'"/></a>';
	} else {
		var img = '<a href="javascript:;" onclick="javascript:att_cancel(this,'+id+',\'upload\')"><div class="icon"></div><img src="statics/images/ext/'+ext+'.png" width="80" imgid="'+id+'" path="'+src+'" title="'+filename+'"/></a>';
	}
...
function att_cancel(obj,id,source){
	...
	if($(obj).hasClass('on')){
		...
		parent.document.getElementById("version").value='';
		parent.document.getElementById("platform").value='0';
		parent.document.getElementById("filesize").value='';
		...
		//自动检测信息并添加到表单yu
		//alert(obj+id+source);
		$.get('index.php?m=attachment&c=attachments&a=package2form&aid='+id,function(data){
			infos = eval('('+data+')');
			parent.document.getElementById("fileid").value=infos.fileid;
			parent.document.getElementById("fileurl").value=infos.fileurl;
			parent.document.getElementById("version").value=infos.version;
			parent.document.getElementById("platform").value=infos.platform;
			parent.document.getElementById("filesize").value=infos.filesize;
			//
		});
	}
}
