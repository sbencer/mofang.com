    ...
    jQuery.fn.imageMaps = function(setting) {
		...
		$container.each(function(){
			...
			$images.each(function(){
				...
				_img_conrainer.append('<div class="button-conrainer"><input class="button" type="button" value="新增热区" /></div>').append('<div class="link-conrainer"></div>').append($.browser.msie ? $('<div class="position-conrainer" style="position:absolute"></div>').css({
					background:'#fff',
					opacity:0
				...
				if(map_name !=''){
					...
					container.find('map[name='+map_name+']').find('area[shape=rect]').each(function(){
						...
						_link_conrainer.append('<p ref="'+index+'" class="map-link"><span class="link-number-text">Link '+index+'</span>: <input class="link_name" type="text" size="60" name="link'+image_param+'[]" value="'+$(this).attr('href')+'" /><input class="link_coords" type="hidden" class="rect-value" name="rect'+image_param+'[]" value="'+coords+'" /></p>');
						coords = coords.split(',');
		...
		$('.button-conrainer input[type=button]').click(function(){
			...
			image = (image == '' ? '' : '['+ image + ']');
			_link_conrainer.append('<p ref="'+index+'" class="map-link"><span class="link-number-text">Link '+index+'</span>: <input class="link_name" type="text" size="60" name="link'+image+'[]" value="" /><input class="link_target" type="checkbox" value="1" checked />新窗口打开<input type="hidden" class="link_coords rect-value" name="rect'+image+'[]" value="10,10,100,40" /></p>');
