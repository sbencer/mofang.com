...
//移除关联文章 by Mofang
function remove_mfrelation(sid, id, field) {
	var input_id = '#relation_' + field;
	var relation_ids = $(input_id).val();
	if(relation_ids !='' ) {
		var r_arr = relation_ids.split('|');
		var newrelation_ids = '';
...
//显示厂商名称
function show_author_name(modelid,id) {
$.getJSON("?m=content&c=content&a=public_getjson_author&modelid="+modelid+"&id="+id, function(json){
	var author_name = '';
	if(json==null) {
		return false;
	}
	$('#authorname').val(json[0].title);
});
}
//移除ID
...
//移除关联文章 by Mofang
function remove_mfrelation_hao(sid, id, field) {
	var input_id = '#relation_' + field;
	var relation_ids = $(input_id).val();
	if(relation_ids !='' ) {
		var r_arr = relation_ids.split('|');
		//var newrelation_ids = '';
		//newrelation_ids = $.grep(r_arr,function(v){return v != '' && v!= id;}).join('|');
		//$(input_id).val('|' + newrelation_ids + '|');
		$(input_id).val('');
		$('#'+sid).remove();
	}
}
//移除关联游戏,ok
function remove_relation_game(sid,id,game_str) {
...
//删除关联的游戏 - 专区用  
function remove_relation_game_partition(relation_id,sid,id) {
	var relation_ids = $('#'+relation_id).val();
	if(relation_ids !='' ) {
		$('#'+sid).remove();
		var r_arr = relation_ids.split('|');
		var newrelation_ids = '';
		...
		$('#'+relation_id).val(newrelation_ids);
	}
}
...
function remove_relation_game_new(sid,id) {
	var relation_ids = $('#relation_game').val();
	if(relation_ids !='' ) {
		$('#'+sid).remove();
		var r_arr = relation_ids.split('|');
		var newrelation_ids = '';
		...
		$('#relation_game').val(newrelation_ids);
	}
}

//显示相关游戏（李伟新产品库）
function show_relation_game(modelid,id) {
$.getJSON("?m=content&c=content&a=public_getjson_relationgame&modelid="+modelid+"&id="+id, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有选择关联游戏');
		return false;
	...
	$.each(json, function(i, n){
		newrelation_ids += "<li id='game"+n.sid+"' style=\"width: 210px;float: left;padding-right: 5px;\">·<span>"+n.title+"</span><a href='javascript:;' class='close' onclick=\"remove_relation_game_new('game"+n.sid+"',"+n.id+")\"></a></li>";
	});

	$('#relation_game_text').html(newrelation_ids);
});
}
