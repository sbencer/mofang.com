...
class model {
	...
	public function __construct() {
		...
		$this->slave_server = $this->db_config[$this->db_setting]['slave_server'];
		...
		if($this->slave_server) {
			...
			// 	$this->read_db = $this->db;
			// } else {
			// 	$this->read_db = db_factory::get_instance($this->db_config)->get_database($slave_server);
			// }
			if(defined('IN_ADMIN') || isset($this->is_master)){
				$this->read_db = $this->db;
			}else{
				$slave_server = weight_rand($this->slave_server);
				if($slave_server=='default') {
	...
	final public function select($where = '', $data = '*', $limit = '', $order = '', $group = '', $key='') {
		...
		return $this->read_db->select($data, $this->table_name, $where, $limit, $order, $group, $key);
	}
	...
	final public function get_one($where = '', $data = '*', $order = '', $group = '') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->read_db->get_one($data, $this->table_name, $where, $order, $group);
	}
	...
	/**
	 * 執行批量添加記錄操作
	 * @param $data 		要增加的數據，參數為二組數組。第二級數組key為字段值，數組值為數據取值
	 * @param $return_insert_id 是否返回新建ID號
	 * @param $replace 是否採用 replace into的方式添加數據
	 * @return boolean
	 */
	final public function batch_insert($data, $return_insert_id = false, $replace = false, $ignore = false) {
		return $this->db->batch_insert($data, $this->table_name, $return_insert_id, $replace, $ignore);
	}
	...
	final public function get_primary() {
		return $this->read_db->get_primary($this->table_name);
	}
	...
	final public function get_fields($table_name = '') {
		...
		return $this->read_db->get_fields($table_name);
	}
	...
	final public function table_exists($table){
		return $this->read_db->table_exists($this->db_tablepre.$table);
	}
	...
	public function field_exists($field) {
		$fields = $this->read_db->get_fields($this->table_name);
		return array_key_exists($field, $fields);
	}

	final public function list_tables() {
		return $this->read_db->list_tables();
	}
	/**
	 * 返回數據結果集
	 ...
	 * @return array
	 */
	final public function fetch_array($key = '') {
		$data = array();
		while($r = $this->read_db->fetch_next()) {
			if ( $key ) {
				$data[$r[$key]] = $r;
			} else {
				$data[] = $r;
