...
class content extends foreground {
	...
	public function publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(!$siteid) $siteid = 1;
			$CATEGORYS = getcache('category_content_'.$siteid, 'commons');
			$priv_db = pc_base::load_model('category_priv_model'); //加載欄目權限表數據模型
	...
	public function delete(){
		...
		if(!$array){
			...
			$table_name = $content_db->table_name;
 			$content_db->delete_content($id); //刪除文章
	...
	public function upload_video() {
		...
		if(empty($setting)) {
			showmessage('上傳功能還在開發中，請稍後重試！');
		}
		$ku6api = new ku6api($setting['sn'], $setting['skey']);
		if (isset($_POST['dosubmit'])) {
			$_POST['info']['catid'] = isset($_POST['info']['catid']) ? intval($_POST['info']['catid']) : showmessage('請選擇欄目！');
			$_POST['info']['title'] = isset($_POST['info']['title']) ? safe_replace($_POST['info']['title']) : showmessage('標題不能為空！');
			...
			if (!$field) showmessage('上傳功能還在開發中，請稍後重試！');
			$_POST['info'][$field] = 1;
			...
			if (is_array($categorys) && !empty($categorys)) {
				...
				if (!$ischild) {
					...
					$cat_list = '<select name="info[catid]" id="catid"><option value="0">請選擇欄目</option>'.$string.'</select>';
				}
			}
			$flash_info = $ku6api->flashuploadparam(); //加載視頻上傳工具信息
