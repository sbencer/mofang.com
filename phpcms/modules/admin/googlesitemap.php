   		...
   		if($dosubmit) {
				...
				if($_POST['mark']) {
 					$baidunum = $_POST['baidunum'] ? intval($_POST['baidunum']) : 20;
  					...
  					$catids = $_POST['catids'];
 					$catid_cache = $this->categorys;//欄目緩存
					$this->content_db = pc_base::load_model('content_model');
 					foreach ($catids as $catid) {
 						$modelid = $catid_cache[$catid]['modelid'];//根據欄目ID查出modelid 進而確定表名,並結合欄目ID:catid 檢索出對應欄目下的新聞條數
 						$this->content_db->set_model($modelid);
 						$result = $this->content_db->select(array('catid'=>$catid,'status'=>99), '*', $limit = "0,$baidunum", 'id desc');
 						//重設表前綴,for循環時用來查,文章正文 
 						...
 						foreach ($result as $arr){
							...
							//根據本條ID,從對應tablename_data取出正文內容
   							$content_arr = $this->content_db->get_one(array('id'=>$id),'content');
   							$content = new_html_special_chars(strip_tags($content_arr['content']));
   							//組合數據
   	 						$smi = $this->baidunews_item($title,$url,$description,$content,$thumb, $keywords,$category,$author,$source,date('Y-m-d', $inputtime));//推薦文件
				...
				foreach($new_model as $modelid=>$m) {//每個模塊取出num條數據 
					...
					foreach ($result as $arr){
						...
						$smi    = $this->google_sitemap_item($url, $today, $content_changefreq, $content_priority);//推薦文件
						$this->add_item2($smi);
					}
				}
				...
				foreach($this->categorys as $catid=>$r) {
					...
					if($r['type'] && $r['child']=='0'){//如果是單網頁並且，沒有子類了
						continue;
 					}
