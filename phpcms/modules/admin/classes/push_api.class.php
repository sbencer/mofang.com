<?php
/**
 *  position_api.class.php 推薦至推薦位接口類
 *
...
class push_api {
	...
	public function __construct() {
		$this->db = pc_base::load_model('position_model');  //加載數據模型
	}
	 ...
	 * @param array $posid 推送到的推薦位ID
	 * @param array $data 推送數據
	 * @param int $expiration 過期時間設置
	 * @param int $undel 是否判斷推薦位去除情況
	 * @param string $model 調取的數據模型
	 * 調用方式
	 * $push = pc_base::load_app_class('push_api','admin');
	 * $push->position_update(323, 25, 45, array(20,21), array('title'=>'文章標題','thumb'=>'縮略圖路徑','inputtime'='時間戳'));
	 */
	 ...
	 * @param int $id 文章id
	 * @param array $input_posid 傳入推薦位數組
	 */
		...
		if ($arr['dosubmit']) {
			...
			if (is_array($arr['posid']) && !empty($arr['posid']) && is_array($param) && !empty($param)) {
				foreach ($arr['posid'] as $pid) {
					$ext = $func_char = '';
					$r = $this->db->get_one(array('posid'=>$pid), 'extention'); //檢查推薦位是否啟用了擴展字段
