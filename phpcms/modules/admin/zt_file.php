...
class zt_file extends admin {
	...
	public function __construct() {
		$this->zt_name = isset($_GET['zt_name']) && trim($_GET['zt_name']) ? str_replace(array('..\\', '../', './', '.\\', '/', '\\'), '', trim($_GET['zt_name'])) : showmessage(L('illegal_operation'), HTTP_REFERER);
		if (empty($this->zt_name)) {
			showmessage('請選擇專區', HTTP_REFERER);
		}
		$this->filepath = PHPCMS_PATH."ztnew".DIRECTORY_SEPARATOR.$this->zt_name.DIRECTORY_SEPARATOR;
		if (file_exists($this->filepath.'config.php')) {
			$this->zt_name_info = include $this->filepath.'config.php';
			if (!isset($this->zt_name_info['name'])) $this->zt_name_info['name'] = $this->zt_name;
		}
	...
	public function init() {
		...
		$local = str_replace(array(PHPCMS_PATH, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('',DIRECTORY_SEPARATOR), $filepath);
		if (substr($local, -1, 1) == '.') {
		...
		$file_explan = $this->zt_name_info['file_explan'];
		$show_header = true;
		$tpl_edit = $this->tpl_edit;
		//獲取當前專區的域名
		$partition_db = pc_base::load_model('partition_model');
		$partition_array = $partition_db->get_one(array("domain_dir"=>$this->zt_name));
		if($partition_array['is_domain']){
			$zt_url = "http://".$partition_array['domain_dir'].".mofang.com/zt/";
		}else{
			$zt_url = "http://www.mofang.com/".$partition_array['domain_dir']."/zt/";
		}
	...
	public function updatefilename() {
		...
		if (!isset($this->zt_name_info['file_explan'])) $this->zt_name_info['file_explan'] = array();
		$this->zt_name_info['file_explan'] = array_merge($this->zt_name_info['file_explan'], $file_explan);
		@file_put_contents($this->filepath.'config.php', '<?php return '.var_export($this->zt_name_info, true).';?>');
	...
	public function edit_file() {
		...
		if ($_POST['dosubmit']) {
			...
			if ($is_write == 1) {
				...
				creat_template_bak($filepath, $this->zt_name, $dir);
	...
	public function visualization() {
		...
		include template($dir,basename($file, '.html'),$this->zt_name);
		$html = ob_get_contents();
		ob_clean();
		pc_base::load_app_func('global');
		$html = visualization($html, $this->zt_name, $dir, $file);
		echo $html;
	}
		...
		switch ($op) {
			...
			case 'json':
				if ($_POST['dosubmit']) {
					...
					if (replace_pc_tag($filepath, $old_pc_tag, $pc_tag, $this->zt_name, $dir)) {
