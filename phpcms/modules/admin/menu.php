...
class menu extends admin {
	 ...
	 * 更新菜單的所屬模式
	 * @param $id INT 菜單的ID
	 * @param $old_data 該菜單的老數據
	 * @param $new_data 菜單的新數據
	 **/
	private function update_menu_models($id, $old_data, $new_data) {
		...
		if (is_array($models_config)) {
			foreach ($models_config as $_k => $_m) { 
				...
				if ($old_data[$_k]==$new_data[$_k]) continue; //數據沒有變化時繼續執行下一項
				$r = $this->db->get_one(array('id'=>$id), 'parentid');
				$this->db->update(array($_k=>$new_data[$_k]), array('id'=>$id));
				if ($new_data[$_k] && $r['parentid']) {
					$this->update_parent_menu_models($r['parentid'], $_k); //如果設置所屬模式，更新父級菜單的所屬模式
				}
			}
		}
		return true;
	...
	private function update_parent_menu_models($id, $field) {
		$id = intval($id);
		$r = $this->db->get_one(array('id'=>$id), 'parentid');
		$this->db->update(array($field=>1), array('id'=>$id)); //修改父級的所屬模式，然後判斷父級是否存在父級
		if ($r['parentid']) {
