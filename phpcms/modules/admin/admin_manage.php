...
class admin_manage extends admin {
	...
	public function init() {
		...
		$infos = $this->db->listinfo('', '', $page, 500);
		$pages = $this->db->pages;
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$admin_fields = array('username', 'email', 'password', 'encrypt','roleid','realname','mf_uid');
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$userid = $info['userid'];
			$admin_fields = array('username', 'email', 'roleid','realname','mf_uid');
			foreach ($info as $k=>$value) {
				if (!in_array($k, $admin_fields)){
					unset($info[$k]);
				}
	...
	public function public_edit_info() {
		$userid = $_SESSION['userid'];
		if(isset($_POST['dosubmit'])) {
			$admin_fields = array('email','realname','lang','avatars','description');
			$info = array();
			$info = $_POST['info'];
