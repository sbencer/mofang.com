	...
	public function role_priv() {
		...
		if(isset($_POST['dosubmit'])){
			...
			if ($siteid) {
				...
				$priv_data = $this->priv_db->select(); //獲取權限表數據
				$modules = 'admin,announce,vote,system,statistics';
		...
		switch ($op) {
			...
			foreach ($category as $k=>$v) {
				if ($v['type'] == 1) {
					...
					$v['commend_check'] = isset($priv[$v['catid']]['commend']) ? 'checked' : '';
				}
			...
			$str = "<tr>
				  ...
				  <td align='center'><input type='checkbox' name='priv[\$catid][]' \$disabled \$commend_check value='commend' ></td>
