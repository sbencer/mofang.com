...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#new_password").formValidator({empty:true,onshow:"<?php echo L('not_change_the_password_please_leave_a_blank')?>",onfocus:"<?php echo L('password').L('between_6_to_20')?>"}).inputValidator({min:6,max:20,onerror:"<?php echo L('password').L('between_6_to_20')?>"}).regexValidator({regexp:"goodpassword",datatype:"enum",onerror:"密碼必須以字母開頭，包含字母、數字、或下劃線"});
