...
<div class="showMsg" style="text-align:center">
    ...
    <div class="content">
	<input type="text" size="41" id="cat_search" value="<?php echo L('quick_search_area');?>" onfocus="if(this.value == this.defaultValue) this.value = ''" onblur="if(this.value.replace(' ','') == '') this.value = this.defaultValue;">
    <ul id="search_div"></ul>
	</div>
</div>
...
<?php if ($_GET['from'] == 'admin'): ?>
	parent.document.getElementById('center_frame').src = '?m=admin&c=area&a=product_tree&pc_hash=<?php echo $_SESSION['pc_hash'];?>';
<?php elseif ($_GET['from'] == 'content'): ?>
	parent.document.getElementById('center_frame').src = '?m=admin&c=area&a=product_tree&pc_hash=<?php echo $_SESSION['pc_hash'];?>';
<?php endif; ?>
...
function(){
	$('#cat_search').keyup(
		function(){
			...
			if (value.length > 0){
				$.getJSON('?m=admin&c=area&a=public_ajax_search', {name: value}, function(data){
					if (data != null) {
						var str = '';
						$.each(data, function(i,n){
							str += '<li><a href="?m=admin&c=area&a=init&productid='+n.id+'&pc_hash='+pc_hash+'">'+n.title+'</a></li>';
						});
						$('#search_div').html(str);
						$('#search_div').show();
					} else {
