 ...
 <tr>
    ...
    <tr>
     <th><?php echo L('issearch');?>：</th>
      <td>
      <input type='radio' name='info[issearch]' value='1' checked> <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;
      <input type='radio' name='info[issearch]' value='0'  > <?php echo L('no');?></td>
    </tr>
    <tr>
    ...
    <tr>
     <th><?php echo L('is_area_menu');?>：</th>
      <td>
      <input type='radio' name='info[is_area_menu]' value='1' > <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;
      <input type='radio' name='info[is_area_menu]' value='0' checked> <?php echo L('no');?></td>
    </tr>
</table>
...
<!--
    ...
    function change_tpl(modelid) {
        if(modelid) {
            $.getJSON('?m=admin&c=category&a=public_change_tpl&modelid='+modelid, function(data){$('#template_list').val(data.template_list);$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);$('#template_m_list').val(data.template_m_list);$('#category_m_template').html(data.category_m_template);$('#list_m_template').html(data.list_m_template);$('#show_m_template').html(data.show_m_template);});
        }
    }
    function load_file_list(id) {
        if(id=='') return false;
        $.getJSON('?m=admin&c=category&a=public_tpl_file_list&style='+id+'&catid=<?php echo $parentid?>', function(data){$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);$('#category_m_template').html(data.category_m_template);$('#list_m_template').html(data.list_m_template);$('#show_m_template').html(data.show_m_template);});
    }
