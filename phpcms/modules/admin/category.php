...
class category extends admin {
	...
	public function init () {
		...
		$show_detail = count($result) < 1500 ? 1 : 0;
	...
	*/
	private function update_url($catid) {
		...
		$url = pc_base::load_app_class('url', 'content'); //調用URL實例
	...
	public function public_tpl_file_list() {
        ...
        if (isset($_GET['issmarty']) && $_GET['issmarty'] == 1) {
            $issmarty = 1;
        }
		pc_base::load_sys_class('form','',0);
		if($_GET['type']==1) {
			...
			$html = array('category_template'=> form::select_template($style, 'content',(isset($cat['setting']['category_template']) && !empty($cat['setting']['category_template']) ? $cat['setting']['category_template'] : 'category'),'name="setting'.$batch_str.'[category_template]"','category',$issmarty),
				'list_template'=>form::select_template($style, 'content',(isset($cat['setting']['list_template']) && !empty($cat['setting']['list_template']) ? $cat['setting']['list_template'] : 'list'),'name="setting'.$batch_str.'[list_template]"','list',$issmarty),
                'show_template'=>form::select_template($style, 'content',(isset($cat['setting']['show_template']) && !empty($cat['setting']['show_template']) ? $cat['setting']['show_template'] : 'show'),'name="setting'.$batch_str.'[show_template]"','show',$issmarty),
                'category_m_template'=> form::select_template($style, 'mobile',(isset($cat['setting']['category_m_template']) && !empty($cat['setting']['category_m_template']) ? $cat['setting']['category_m_template'] : 'category'),'name="setting'.$batch_str.'[category_m_template]"','category',$issmarty),
				'list_m_template'=>form::select_template($style, 'mobile',(isset($cat['setting']['list_m_template']) && !empty($cat['setting']['list_m_template']) ? $cat['setting']['list_m_template'] : 'list'),'name="setting'.$batch_str.'[list_m_template]"','list',$issmarty),
				'show_m_template'=>form::select_template($style, 'mobile',(isset($cat['setting']['show_m_template']) && !empty($cat['setting']['show_m_template']) ? $cat['setting']['show_m_template'] : 'show'),'name="setting'.$batch_str.'[show_m_template]"','show',$issmarty)
			);
		}
		if ($_GET['module']) {
			unset($html);
			if ($_GET['templates']) {
				...
				if (is_array($templates)) {
					foreach ($templates as $k => $tem) {
						$t = $tem.'_template';
						if ($id[$k]=='') $id[$k] = $tem;
						$html[$t] = form::select_template($style, $_GET['module'], $id[$k], 'name="'.$_GET['name'].'['.$t.']" id="'.$t.'"', $tem, $issmarty);
					}
				}
			}
	 ...
	 * json方式加載模板
	 */
	public function public_tpl_file_list_partition() {

		$catid = $_GET['en_part'] ? $_GET['en_part']:'';

		//篩選模板
		$db_partition = pc_base::load_model('partition_model');
		$arrparentid = $db_partition->get_one("`catid`='".$catid."'", "arrparentid");
		$arrparentid = explode(',',$arrparentid['arrparentid']);
		$parentid = $arrparentid[1] ? $arrparentid[1] : $catid;
		$temp_domain_dir = $db_partition->get_one("`catid`='".$parentid."'", "domain_dir");
		$en_partition = $temp_domain_dir['domain_dir'];

		$tpl_root = pc_base::load_config('system', 'tpl_root');
		$templatedir = PC_PATH.$tpl_root.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.'partition'.DIRECTORY_SEPARATOR;
		$templates = glob($templatedir.$en_partition.'*.html');
		...
		$partition_info = $db_partition->get_one("`catid`=".$catid, "setting");
		$template_info = string2array($partition_info['setting']);

		$category_option = '<option>請選擇</option>';
		foreach( $templates as $key=>$value ){
			$category_selected = $template_info['category_template']==$key ? 'selected' : '';
			$category_option .= '<option value="'.$key.'" '.$category_selected.'>'.$value.'</option>';
		}
		$list_option = '<option>請選擇</option>';
		foreach( $templates as $key=>$value ){
			$list_selected = $template_info['list_template']==$key ? 'selected' : '';
			$list_option .= '<option value="'.$key.'" '.$list_selected.'>'.$value.'</option>';
		}
		$show_option = '<option>請選擇</option>';
		foreach( $templates as $key=>$value ){
			$show_selected = $template_info['show_template']==$key ? 'selected' : '';
			$show_option .= '<option value="'.$key.'" '.$show_selected.'>'.$value.'</option>';
		}
		$html = array(
			'category_template'=> '<select name="setting[category_template]">'.$category_option.'</select>',
			'list_template'=>'<select name="setting[list_template]">'.$list_option.'</select>',
			'show_template'=>'<select name="setting[show_template]">'.$show_option.'</select>'
		);
	 ...
	 * json方式讀取風格列表，推送部分調用
	 */
	public function public_change_tpl_partition() {

		$en_partition = $_GET['en_partition'] ? $_GET['en_partition']:'';
		$catid = $_GET['catid'] ? $_GET['catid']:0;

		//默認選擇模板頁
		//使用欄目/專區的默認頁作為當前新建項的默認選中項
		$default_tem = array();
		if( $catid ){
			$db_partition = pc_base::load_model('partition_model');
			$arrparentid = $db_partition->get_one("`catid`='".$catid."'", "arrparentid");
			...
			$father_id = $catid;
			if( $father_id ){
				$default_tem = $db_partition->get_one("`catid`='".$father_id."'", "setting");
				$default_tem = string2array($default_tem['setting']);
			}
		}
		...
		$category_option = '<option>請選擇</option>';
		foreach( $templates as $key=>$value ){
			$category_selected = $default_tem['category_template']==$key ? 'selected' : '';
			$category_option .= '<option value="'.$key.'" '.$category_selected.'>'.$value.'</option>';
		}
		$list_option = '<option>請選擇</option>';
		foreach( $templates as $key=>$value ){
			$list_selected = $default_tem['list_template']==$key ? 'selected' : '';
			$list_option .= '<option value="'.$key.'" '.$list_selected.'>'.$value.'</option>';
		}
		$show_option = '<option>請選擇</option>';
		foreach( $templates as $key=>$value ){
			$show_selected = $default_tem['show_template']==$key ? 'selected' : '';
			$show_option .= '<option value="'.$key.'" '.$show_selected.'>'.$value.'</option>';
		}
		$html = array(
			'category_template'=> '<select name="setting[category_template]">'.$category_option.'</select>',
			'list_template'=>'<select name="setting[list_template]">'.$list_option.'</select>',
			'show_template'=>'<select name="setting[show_template]">'.$show_option.'</select>',
			'other_template'=>'<select name="setting[other_template]">'.$option.'</select>'
