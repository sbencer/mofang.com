...
<?php } else {?>
<span class="font-fixh green">未綁定平台賬戶，請點擊<a href="index.php?m=sms&c=sms&a=sms_setting&menuid=1539&pc_hash=<?php echo $_GET['pc_hash'];?>"><span class="font-fixh">平台設置</span></a>綁定。</span>
<?php }?>
</div><br>

<div class="btn text-l">
<span class="font-fixh green">當前服務器IP為 ： <span class="font-fixh"><?php echo $_SERVER["SERVER_ADDR"];?></span> <?php if(!empty($smsinfo_arr['allow_send_ip']) &&!in_array($_SERVER["SERVER_ADDR"],$smsinfo_arr['allow_send_ip'])) echo '當前服務器所在IP不允許發送短信';?>
</div>
<br>
...
<div class="explain-col search-form">
開啟會員注冊短信驗證方法：後台->用戶->會員模塊配置->手機強制驗證方式 選擇 <font color="red">是</font>
</div>
</div>
</div>
<br>
