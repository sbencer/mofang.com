...
<?php echo L('時間段查看統計');?>  <?php echo form::date('start_time',$start_time)?>  至 <?php echo form::date('end_time',$end_time)?> <input type="submit" value="<?php echo L('search')?>" class="button" name="dosubmit"> 
...
?>   
	...
	<td align="center"><a href="javascript:void(0);" onclick="view_video_stat('<?php echo $info['vid']?>','<?php echo $info['title'];?>')"><?php echo L('查看詳情')?></a>  </td>
	</tr>
<?php 
	}
}
...
function view_video_stat(vid,title) {
	window.top.art.dialog({title:title+' 視頻近30日播放量走勢圖', id:'view', iframe:'?m=video&c=stat&a=show_video_stat&vid='+vid ,width:'780px',height:'450px'});
}
//總體趨勢圖
function video_vv_trend() {
	window.top.art.dialog({title:'近20日視頻總體播放量走勢圖', id:'view', iframe:'?m=video&c=stat&a=vv_trend' ,width:'890px',height:'450px'});
}
//提交指定URL
