...
　<?php if(isset($big_menu)) { foreach($big_menu as $big) { echo '<a class="add fb" href="'.$big[0].'"><em>'.$big[1].'</em></a>　'; } }?>&nbsp;<a <?php if(!$_GET['user']) echo 'class="on"';?> href="?m=special&c=special"><em><?php echo L('special_list')?></em></a>&nbsp;&nbsp;&nbsp;<a <?php if($_GET['user']) echo 'class="on"';?>href="?m=special&c=content&a=init&specialid=<?php echo $_GET['specialid'];?>&user=<?php echo $admin_username;?>"><em><?php echo L('my_content')?></em></a></div>
</div>
<div class="pad-10">
