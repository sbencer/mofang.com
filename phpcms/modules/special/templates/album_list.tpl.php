...
<table width="100%" cellspacing="0" class="search-form">
    <tbody>
		...
		<div class="explain-col">
				...
				<?php echo form::select($ku6channels, $_GET['categoryid'], 'name="categoryid" id="categoryid"', '請選擇頻道')?>			
				標題：
...
?>   
	...
	<p class="gray4">播放次數：<?php echo $info['videocount']?></p>
	</div>
	</td>
	<td align="center"><span style="height:22"><?php if (!empty($imported) && in_array($info['id'], $imported)){?><font color="red">專輯已載入<?php } else {?><a href='?m=special&c=album&a=import&id=<?php echo $info['id']?>&page=<?php echo $_GET['page'];?>&pc_hash=<?php echo $_GET['pc_hash'];?>'>載入此專輯</a><?php }?></span><br /><span style="height:22"><a href="?m=special&c=album&a=content_list&id=<?php echo $info['id']?>&menuid=<?php echo $_GET['menuid']?>&pc_hash=<?php echo $_GET['pc_hash'];?>">查看專輯下的視頻</a></span></td>
	</tr>
<?php 
	}
}
?>
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all')?>/<?php echo L('cancel')?></label>
        <input name='dosubmit' type='submit' class="button" value='載入'>&nbsp;
       </div>
