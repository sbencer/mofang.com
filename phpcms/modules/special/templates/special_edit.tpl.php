...
<div id="div_setting_2" class="contentList pad-10 hidden">
	<table width="100%" class="table_form ">
        ...
        <tr>
			<th align="right"  valign="top"><?php echo 'Smarty模板';?>：</th>
			<td valign="top"><?php echo form::radio(array('0'=>L('no'), '1'=>L('yes')), $info['issmarty'], 'name="special[issmarty]"', '', 'smarty');?></td>
		</tr>
		<tr>
		    <th align="right"  valign="top"><?php echo L('template_style')?>：</th>
            <td valign="top"><?php echo form::select($template_list, $info['style'], 'name="special[style]" id="style" onchange="load_file_list(this.value)"', L('please_select'))?><?php if ($info['style']) {?><script type="text/javascript">$.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=<?php echo $info['style']?>&module=special&templates=index|list|show&id=<?php echo $info['index_template']?>|<?php echo $info['list_template']?>|<?php echo $info['show_template']?>&name=special&issmarty=<?php echo $info['issmarty']?>', function(data){$('#index_template').html(data.index_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});</script><?php }?></td>
		</tr>
		<tr>
...
function load_file_list(id) {
    if($('#smarty_1').attr('checked')){
        $.getJSON('?m=admin&c=category&a=public_tpl_file_list&style='+id+'&module=special&templates=index|list|show&name=special&issmarty=1', function(data){$('#index_template').html(data.index_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
    } else {
