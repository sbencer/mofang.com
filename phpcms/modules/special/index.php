...
class index {
	...
	public function init() {
        ...
        if ($info['issmarty']) {
            ...
            $smarty->display('special/'.$template.'.tpl');
        } else {
	...
	public function type() {
        ...
        if (strtolower($_GET['format']) == 'json') {
            include template('special', 'type_json');
        } else {
            include template('special', $template);
        }
	}
	...
	public function show() {
		...
		if ($paginationtype) { 			//文章使用分頁時
			if($paginationtype==1) {
				...
				$content = $contentpage->get_data($content, $maxcharperpage); //自動分頁，自動添加上[page]
			} 
		} else {
			if (strpos($content, '[/page]')!==false) {
				$content = preg_replace("|\[page\](.*)\[/page\]|U", '', $content);
		...
		$template = $show_template ? $show_template : 'show'; //調用模板
        ...
        if (is_mobile()) {
            include template('content','html5', 'ios');
        } else {
