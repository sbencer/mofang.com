...
class html {
	...
	public function _create_content($contentid = 0) {
		...
		if ($arr_content['paginationtype']) {			//文章使用分頁時
			if($arr_content['paginationtype']==1) {
				...
				$contentpage = pc_base::load_app_class('contentpage', 'content'); //調用自動分頁類
				$arr_content['content'] = $contentpage->get_data($arr_content['content'], $arr_content['maxcharperpage']); //自動分頁，自動添加上[page]
			} 
		} else {
			if (strpos($arr_content['content'], '[/page]')!==false) {
				$arr_content['content'] = preg_replace("|\[page\](.*)\[/page\]|U", '', $arr_content['content']);
		...
		$template = $arr_content['show_template'] ? $arr_content['show_template'] : 'show'; //調用模板
		...
		if ($CONTENT_POS !== false) {
			...
			for ($i=1; $i<=$pagenumber; $i++) {
				...
				//如果是分站的文件，將文件寫入到信息隊列中
				$this->queue->add_queue('add', $file, $siteid);
				$file = PHPCMS_PATH.$file; //生成文件的路徑
		...
		//$this->_index($specialid, 20, 5);  //更新專題首頁
		//$this->_list($typeid, 20, 5); 		//更新所在的分類頁
		return $urls;
	}
	...
	public function _index($specialid = 0, $pagesize = 20, $pages_num = 0) {
		...
		if($ispage) {
			...
			for ($i=1; $i<=$pages_num; $i++) {
				...
				$this->queue->add_queue('add', $file_root, $siteid); //添加至信息隊列
			...
			$this->queue->add_queue('add', $file, $siteid); //添加至信息隊列
			$file = PHPCMS_PATH.$file;
			ob_start();
	 ...
	 * 生成分類靜態頁
	 * @param intval $typeid 分類ID
	 * @param intval $pagesize 每頁篇數
	 * @param intval $pages 最大更新頁數
 	 */
