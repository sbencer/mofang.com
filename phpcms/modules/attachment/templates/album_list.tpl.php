...
<li>
	<div class="img-wrap">
		<a href="javascript:;" onclick="javascript:album_cancel(this,'<?php echo $r['aid']?>','<?php echo $r['filepath']?>')"><div class="icon"></div><img src="<?php echo $r['src']?>" width="<?php echo $r['width']?>" path="<?php echo $r['filepath']?>" title="<?php echo $r['filename']?>"/></a>
	</div>
</li>
<?php } ?>
</ul>
...
function album_cancel(obj,id,source){
	...
	if($(obj).hasClass('on')){
		...
		if(num > file_upload_limit) {alert('不能選擇超過'+file_upload_limit+'個附件'); return false;}
		$(obj).addClass("on");
