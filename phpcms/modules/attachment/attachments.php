...
class attachments {
	...
	public function swfupload(){
		...
		if(isset($_POST['dosubmit'])){
			...
			//檢測是否為軟件包，使用自定義模板yu	
			$pattern = '/apk|xap|ipa/i';
			$is_app = preg_match($pattern, $args);	
			if($is_app){
				include $this->admin_tpl('appupload');
			}else{
	...
	public function album_dir() {
		...
		$url = ($dir == '.' || $dir=='') ? $this->upload_url : str_replace('.', '', $dir).'/';
		$show_header = true;
		include $this->admin_tpl('album_dir');
	}
	
	/**
	 * 設置upload上傳的json格式cookie
	 */
	...
	//包上傳時將自動填充數據獲得yu
	function package2form(){
		$this->package_db= pc_base::load_model('app_package_model');
		$data = '`id`,`size`,`filepath`,`infos`';
		$where['id'] = $_GET['aid'];
		$base = $this->package_db->get_one($where,$data);
		$infos = json_decode($base['infos'],true);
		$info['fileid'] = $base['id'];
		$info['filesize'] = sizecount($base['size']);
		$info['fileurl'] = pc_base::load_config('system','upload_url').$base['filepath'];
		$info['version'] = $infos['Version'];
		$info['platform'] = $infos['PlatForm'];
		echo json_encode($info);
	}
	
	//檢測遠程文件是否有效
	function check_romote(){
		$url = $_GET['data'];
		$ch = curl_init();
		...
		$ret = curl_exec($ch);
		//獲取狀態碼和文件大小
		$check['status'] = curl_getinfo($ch,CURLINFO_HTTP_CODE);
		$check['size'] = sizecount(curl_getinfo($ch,CURLINFO_CONTENT_LENGTH_DOWNLOAD));
		curl_close($ch);
