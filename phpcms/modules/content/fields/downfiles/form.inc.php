	function downfiles($field, $value, $fieldinfo) {
		...
		if($value) {
			...
			if(is_array($value)) {
                ...
                $list_str .= "<div id='multifile{$_k}'><input type='text' name='{$field}_fileurl[]' value='{$_v}' style='width:310px;' class='input-text'> <input type='text' name='{$field}_filename[]' value='{$_k}' style='width:160px;' class='input-text'> <a href=\"javascript:remove_div('multifile{$_k}')\">".L('remove_out')."</a></div>";
				}
			}
		}
