			...
			if($_GET['a']=='add') {
				...
				foreach($value as $r) {
					if($r!='-1') {
						...
						$position_data_db->insert(array('id'=>$this->id,'catid'=>$this->data['catid'],'posid'=>$r,'thumb'=>$thumb,'module'=>'content','modelid'=>$this->modelid,'data'=>$textcontent,'listorder'=>$this->id,'inputtime'=>$inputtime));
					}
				}
			} else {
				$posids = array();
