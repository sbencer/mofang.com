...
<div class="table-list">
	...
	<tr onclick="select_list(this,'<?php echo safe_replace($r['name']);?>',<?php echo $r['id'];?>)" class="cu" title="<?php echo L('click_to_select');?>">
		...
		<td align='center'><a href="http://game.mofang.com.tw/info/<?php echo $r['id'];?>.html" target="_blank" title="點擊查看此遊戲詳情！"><img src="<?php echo $r['icon'];?>" style="width:60px;"></a></td>
...
<!--
	function select_list(obj,title,id) {
		var relation_ids = window.top.$('#relation_game').val();
		...
		if($(obj).attr('class')=='line_ff9966' || $(obj).attr('class')==null) {
			...
			if(relation_ids !='' ) {
				...
				window.top.$('#relation_game').val(newrelation_ids);
			}
		} else {
			$(obj).attr('class','line_ff9966');
			var str = "<li id='"+sid+"' style=\"width: 210px;float: left;padding-right: 5px;\">·<span>"+title+"</span><a href='javascript:;' class='close' onclick=\"remove_relation_game_new('"+sid+"',"+id+")\"></a></li>";
			window.top.$('#relation_game_text').append(str);
			if(relation_ids =='' ) {
				window.top.$('#relation_game').val(id);
			} else {
				relation_ids = relation_ids+'|'+id;
				window.top.$('#relation_game').val(relation_ids);
			}
		}
}
//-->
