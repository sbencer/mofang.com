...
<a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>&steps=21&pc_hash=<?php echo $pc_hash;?>" <?php if($steps==21 && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('on_timed');?></em></a><span>|</span>
<a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>&user=<?php echo $admin_username; ?>&pc_hash=<?php echo $pc_hash;?>" <?php if(isset($_GET['user']) && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('my_content');?></em></a><span>|</span>
<?php if(in_array($_SESSION['roleid'], array(1,2,4,5,9))) {?>
<a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>&checktags=check&pc_hash=<?php echo $pc_hash;?>" <?php if($_GET['checktags']=='check' && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('check_tags');?></em></a><span>|</span>
<?php }?>
...
<?php }?>
<span>|</span>
<a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>&steps=33&pc_hash=<?php echo $pc_hash;?>" <?php if(isset($_GET['user']) && !isset($_GET['reject'])) echo 'class=on';?>>
	<em>回收站</em>
</a>
</div>
...
<?php if (!in_array($modelid, array(23))): ?>
            <th width="150">FB-UPDATE</th>
<?php endif; ?>
<?php if ($exists_gameid||$relation_game): ?>
            <th width="48">關聯遊戲</th>
<?php endif; ?>
...
<tbody>
	...
	if(is_array($datas)) {
		...
		$this->relation_game = pc_base::load_model('relation_game_model');

		foreach ($datas as $r) {
			$hits_r = $this->hits_db->get_one(array('hitsid'=>'c-'.$modelid.'-'.$r['id']));
			//判斷是否關聯新庫
			$relation_check = $this->relation_game->get_one(array("catid"=>$r['catid'],"id"=>$r['id']),'gameid');
	?>
        <tr>
...
<?php if (!in_array($modelid, array(23))): ?>
<td align='center'><a href='https://developers.facebook.com/tools/debug/og/echo?q=<?php echo urlencode($r['url']);?>' target='_blank'>FB-UPDATE</a></td>
<?php endif; ?>
<?php if ($exists_gameid||$relation_game): ?>
		<td align='center'><?php if ($relation_check):?><span class='green'>是</span><?php else: ?><span class='red'>否</span><?php endif;?></td>
<?php endif; ?>
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
		...
		<?php if($status==99) {?>
			<input type="button" class="button" value="<?php echo L('delete');?>" onclick="myform.action='?m=content&c=content&a=delete_new&dosubmit=1&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';return confirm_delete('確定放到回收站裡嗎？')"/>
		<?php }else{ ?>
			<input type="button" class="button" value="<?php echo L('delete');?>" onclick="myform.action='?m=content&c=content&a=delete&dosubmit=1&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';return confirm_delete('真的...確定徹底刪除？')"/>

		<?php } ?>


		<?php if(!isset($_GET['reject']) && $status!=33) { ?>
