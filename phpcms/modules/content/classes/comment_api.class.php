...
class comment_api {
	...
	function get_info($module, $contentid, $siteid) {
		...
		$r = $this->db->get_one(array('catid'=>$catid, 'id'=>$contentid), '`title`,`keywords`,`description`');
		...
		if ($r) {
			return array('title'=>$r['title'], 'keywords'=>$r['keywords'], 'description'=>$r['description'],'url'=>go($catid, $contentid, 1), 'allow_comment'=>(isset($data_info['allow_comment']) ? $data_info['allow_comment'] : 1));
		} else {
			return false;
		}
	}
