		...
		if (count($this->getItemList()) > 0) {
			foreach ($this->getItemList() as $id) {
				...
				if (strlen($item->getTitle()) > 0 && strlen($item->getLink()) > 0) {
					...
					if (strlen($item->getImage()) > 0) {
						$this->output .= (string) '<image>' . $item->getImage() . '</image>' . "\n";
					} // end if

					//以下新增加ID，catname , gamename 3个字段
					if ($item->getId()) {
						$this->output .= (string) '<id>' . $item->getId() . '</id>' . "\n";
					} // end if

					if (strlen($item->getCatname()) > 0) {
						$this->output .= (string) '<catname>' . $item->getCatname() . '</catname>' . "\n";
					} // end if

					if (strlen($item->getGamename()) > 0) {
						$this->output .= (string) '<gamename>' . $item->getGamename() . '</gamename>' . "\n";
					} // end if
...
class RSSItem extends RSSBase {
	...
	*/
	...
	function setId($id = '') {
		if (!isset($this->id) && strlen($id) > 0) {
			$this->id = $id;
		} // end if
	} // end function

	function setCatname($catname = '') {
		if (!isset($this->catname) && strlen(trim($catname)) > 0) {
			$this->catname = (string) trim($catname);
		} // end if
	} // end function

	function setGamename($gamename = '') {
		if (!isset($this->gamename) && strlen(trim($gamename)) > 0) {
			$this->gamename = (string) trim($gamename);
		} // end if
	} // end function
