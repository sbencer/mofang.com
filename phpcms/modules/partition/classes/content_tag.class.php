...
class content_tag {
	...
	public function iosgame_count($data) {
		...
		$sql  = sprintf("SELECT COUNT(*) as cnt FROM %s m, %s d WHERE %s",
			$this->db->table_name,
	...
	public function gameinfo_lists($data) {
		...
		if(!$this->set_modelid($catids[0])) return false;
	...
	public function lists($data) {
		...
		$order = $data['order'];
		if(!empty($data['limitd'])) $data['limit'] = $data['limitd'];	
	...
	public function iosgame_lists($data) {
		...
		//$return = $this->db->select($sql, '*', $data['limit'], $order, '', 'id');
		$sql  = sprintf("SELECT %s FROM %s m, %s d, %s h WHERE %s ORDER BY %s LIMIT %s",
			"m.id, m.title, m.stars, m.icon, m.url, m.keywords, m.description, d.package, d.brief, d.version,d.icon_tag, d.price_number, d.price_unit, d.filesize, d.limit_free, d.limit_free_timeline, d.release_time, h.views",
			$this->db->table_name,
			$this->db->table_name . '_data',
			$this->db->db_tablepre . 'hits',
			$sql,
			$order,
			$data['limit']);
		$res = $this->db->query($sql);
		...
		if (!$ids) return NULL;
		$sql = sprintf("SELECT tag_id, content_id, tag_name, parent_id FROM %slinktag t INNER JOIN %slinktag_to_content lt ON t.tag_id = lt.linktag_id WHERE catid = %d AND content_id IN (%s)",
			$this->db->db_tablepre,
			$this->db->db_tablepre,
			$catid,
			join(',', $ids));
