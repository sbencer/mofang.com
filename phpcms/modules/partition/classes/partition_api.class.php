...
class partition_api {
	...
	public function __construct() {
		...
		//專區分類模型,ok
		//$this->partition_db = pc_base::load_model('category_model');
		$this->partition_db = pc_base::load_model('partition_model');
		//分區遊戲模型,notok->待整理
		$this->partition_games_db = pc_base::load_model('partition_games_model');
	}
	...
	/**
	 * 調取專區附屬分類,ok
	 * @param intval $partitionid 專區ID
	 * @return array 專區的附屬分類
	 */
	public function _get_types($partitionid = 0) {
		if (!$partitionid) return false;
		//$rs = $this->partition_db->select(array('parentid'=>$partitionid, 'siteid'=>get_siteid()), 'catid, catname');
		$rs = $this->partition_db->select(array('catid'=>$partitionid, 'siteid'=>get_siteid()), 'catid, catname');
		$types = array();
		foreach ($rs as $r) {
			$types[$r['catid']] = $r['catname'];
		}
		return $types;
	}
	 ...
	 * 導入的數據添加到數據表
	 * @param intval $modelid	 模型ID
	 * @param intval $partitionid	 信息的所屬專區ID
	 * @param intval $id 		 信息的ID
	 * @param intval $typeid 	 信息的分類ID
	 * @param intval $listorder	 信息的排序
	 */
	public function _import($modelid, $partitionid, $id, $typeid, $listorder = 0) {

		//if (!$specialid || !$id || !$typeid) return false;
		if (!$partitionid || !$id) return false;
		$c = pc_base::load_model('content_model');
		$c->set_model($modelid);

		/*
		$info = $c->get_one(array('id'=>$id, 'status'=>99), '`id`, `catid`, `title`, `thumb`, `url`, `description`, `username`, `inputtime`');
		if ($info) {
			...
			if(!$this->c_db->get_one(array('title'=>addslashes($info['title']), 'specialid'=>$specialid, 'typeid'=>$typeid))) {
				...
				return $this->partition_games_db->insert($info, true);
			}
		}*/

		$info = $c->get_one(array('id'=>$id, 'status'=>99), '`id`, `catid`, `inputtime`');
		if ($info) {
			if(!$this->partition_games_db->get_one(array('modelid'=>$modelid, 'part_id'=>$partitionid, 'gameid'=>$id))) {
				//$info['part_id'] = $partitionid;
				//$info['typeid'] = $typeid;
				$game_info['modelid'] = $modelid;
				$game_info['part_id'] = $partitionid;
				$game_info['gameid'] = $id;

				$game_info['inputtime'] = $info['inputtime'];

				//return $this->c_db->insert($info, true);
				return $this->partition_games_db->insert($game_info, true);
			}
		}
