...
class url2{
	...
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0) {
		...
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			...
			//$urlrules = 'index.php?m=partition&c=index&a=show&p='.$_GET['p'].'&catid={$catid}&id={$id}|index.php?m=partition&c=index&a=show&&p='.$_GET['p'].'&catid={$catid}&id={$id}&page={$page}';
			$urlrules = partition_url().'{$catid}_{$id}_1.html|'.partition_url().'{$catid}_{$id}_{$page}.html';
			...
			if($content_ishtml && $url) {
				if ($domain_dir && $category['isdomain']) {
					...
					$url_arr[0] = partition_url() == '/' ? $match_url.$html_root.'/'.$urls : $match_url.rtrim(partition_url(),'/').$html_root.'/'.$urls;
				}
			} elseif($content_ishtml) {
				$url_arr[0] = partition_url() == '/' ? $html_root.'/'.$urls : rtrim(partition_url(),'/').$html_root.'/'.$urls;
				$url_arr[1] = $html_root.'/'.$urls;
			} else {
                if (substr($urls, 0, 7) == 'http://') {
                    $url_arr[0] = $url_arr[1] = $urls;
                } else {
                    $url_arr[0] = $url_arr[1] = partition_url().$urls;
                }
			}
		}
		//生成靜態 ,在添加文章的時候，同時生成靜態，不在批量更新URL處調用
	...
	public function category_url($catid, $page = 1) {
		...
		if (!$setting['ishtml']) { //如果不生成靜態
			...
			$parentids[] = $catid;
			$domain_dir = '';
			foreach ($parentids as $pid) { //循環查詢父欄目是否設置了二級域名
				$r = $this->categorys[$pid];
				if (strpos(strtolower($r['url']), '://')!==false && strpos($r['url'], '?')===false) {
					$r['url'] = preg_replace('/([(http|https):\/\/]{0,})([^\/]*)([\/]{1,})/i', '$1$2/', $r['url'], -1); //取消掉雙'/'情況
					if (substr_count($r['url'], '/')==3 && substr($r['url'],-1,1)=='/') { //如果url中包含‘http://’並且‘/’在3個則為二級域名設置欄目
						$url = $r['url'];
						$domain_dir = $this->get_categorydir($pid).$this->categorys[$pid]['catdir'].'/'; //得到二級域名的目錄
					}
				}
			}
			...
			if ($url && $domain_dir) { //如果存在設置二級域名的情況
