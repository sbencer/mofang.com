...
<div class="subnav">
	...
	<?php if( $catid || $parentid ){?>

		<?php if( ($parentid && $act=='edit') || $nav_add ){?>
			<a href="?m=partition&c=partition&a=init&pc_hash=<?php echo $_SESSION['pc_hash'];?>" ><em>返回專區列表</em></a>
    			<span>|</span>
		<?php } ?>
		<?php if( $nav_pid_istop || $nav_add  ){?>
			<a href="?m=partition&c=partition&a=init&parentid=<?php echo $nav_pid[1];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>" ><em>返回欄目列表</em></a>
		<?php }?>
	<?php }?>
    </div>
</div>
