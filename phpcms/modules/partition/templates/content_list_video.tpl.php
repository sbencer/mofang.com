...
$.fn.inputlimiter = function () {
    ...
    $(this).each(function (i) {
		...
		if ($(this).nextAll('div.textlimit').size() == 0) {
            $(this).after('<div class="textlimit">還可輸入 n 字符</div>');
        }
		*/
        $(this).bind('propertychange focus keyup input paste', function () {
            ...
            $(this).nextAll('div.textlimit').html('還可輸入 ' + (_max-_length) + ' 字符');
...
function upthumb(fid) {
	...
	});window.top.art.dialog({id:'update_thumb'}).close();} else {alert('請先選擇縮略圖！');return false;}}, function(){window.top.art.dialog({id:'update_thumb'}).close()});
}
