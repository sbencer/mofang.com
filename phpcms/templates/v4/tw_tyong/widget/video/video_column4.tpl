<div class="zq_video clearfix">
	<div class="nav-common title2-bg mb10">
		<a href="{get_part_url($module_setting_type['video/video_column4']['video_arr'])}" class="more-common fr">更多<em>&gt;</em></a>
		<ul class="nav-com-list clearfix">
			<li class="class1">
			   {$module_setting_type['video/video_column4']['video_title']}
			</li>
		</ul>
	</div>
    <!--video_list start-->
    <ul class="zq_video_list have_big clearfix">
    {pc M=partition action=partition_contents partid={$module_setting_type['video/video_column4']['video_arr']} makeurl=1 fields='id,catid,url,title,description,inputtime,thumb' nums=12}
        {foreach $data as $val}
