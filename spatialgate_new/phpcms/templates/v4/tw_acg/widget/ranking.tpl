<div class="hot-ranking clearfix">
	...
	<div class="tab_con">
		...
		{pc M="content" action="hits" catid="6" num="5" order="views DESC" cache="3600"}
			{foreach $data as $val}
		...
		{pc M="content" action="hits" catid="21" num="5" order="views DESC" cache="3600"}
			{foreach $data as $val}
			<li><a href="{$val.url}">{$val@iteration}.{$val.title}</a></li>
			{/foreach}
		{/pc}
		...
		<ul class="article_list disno">
		{pc M="content" action="hits" catid="9" num="5" order="views DESC" cache="3600"}
			{foreach $data as $val}
			<li><a href="{$val.url}">{$val@iteration}.{$val.title}</a></li>
			{/foreach}
		{/pc}
