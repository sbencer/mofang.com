...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 7) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			$multipage .= ' <a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="page_prev">'.L('previous').'</a>';
			if($curr_page==1) {
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= '<span>..</span><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="page_next">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="page_next">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span>'.$pages.'</span> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="page_next">'.L('next').'</a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="page_next">'.L('next').'</a>';
		}
	}
...
function seo($siteid, $catid = '', $title = '', $description = '', $keyword = '') {
	...
	$seo['title'] =  (isset($title) && !empty($title) ? $title.' - ' : '').(isset($cat['setting']['meta_title']) && !empty($cat['setting']['meta_title']) ? $cat['setting']['meta_title'].' - '.$seo['site_title'] : (isset($cat['catname']) && !empty($cat['catname']) ? $cat['catname'].' - '.$seo['site_title'] : $seo['site_title'].' '));
	foreach ($seo as $k=>$v) {
