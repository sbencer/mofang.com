...
class index {
	...
	function __construct() {
        ...
        $this->_cached_action = array('init', 'lists', 'show', 'ajax_lists');
        ...
        if ( !$this->_disable_cache && in_array(ROUTE_A, $this->_cached_action) ) {
            ...
            $html = getcache($this->_cache_key, '', 'memcache', 'html');
	...
	public function init() {
        ...
        if($this->_is_wap) {
		    $smarty->display('tw_acg/wap/index.tpl');
        } else {
		    $smarty->display('tw_acg/index.tpl');
        }
	...
	public function show() {
        ...
        if($this->_is_wap) {
		    $smarty->display('tw_acg/wap/show.tpl');
        } else {
            $templates = STYLE.DIRECTORY_SEPARATOR.$template;
            if($smarty->templateExists($templates)) {
                $smarty->display($templates);
                return;
            }
		    $smarty->display('tw_acg/show.tpl');
        }
	...
	public function lists() {
		...
		$template = $setting['category_template'] ? $setting['category_template'] : 'category.tpl';
		$template_list = $setting['list_template'] ? $setting['list_template'] : 'list.tpl';
		...
		if($type==0) {
            ...
            if($this->_is_wap) {
                if($catid == 8) {
                    $smarty->display('tw_acg/wap/photo.tpl');
                } else {
                    $smarty->display('tw_acg/wap/list.tpl');
                }
            } else {
                $templates = STYLE.DIRECTORY_SEPARATOR.$template;
                if($smarty->templateExists($templates)) {
                ...
                if($modelid == 3) {
                    $smarty->display('tw_acg/list_pic.tpl');
                } else {
                    $smarty->display('tw_acg/list.tpl');
                }
            }
	...
	public function ajax_lists() {
		...
		foreach ($lists as $k=>$v) {
            ...
            if($_GET['date_format'] == 1) {
				$lists[$k]['inputtime'] = date("Y年m月d日 H:i",$v['inputtime']);
            } else {
				$lists[$k]['inputtime'] = date("Y-m-d",$v['inputtime']);
            }
            // 附加信息，添加文章閱讀數查詢
            if(in_array('views', $expand)) {
                $lists[$k]['views'] = get_views("c-{$modelid}-{$v['id']}");    
            }
		}
		...
		switch($_GET['type']) {
			case 'news':
				$smarty = new CSmarty();
				$smarty->assign('news', $lists);
				echo $html = $smarty->fetch('tw_acg/widget/wap/list_json.tpl');
				break;
			case 'video':
				$smarty = new CSmarty();
				$smarty->assign('video', $lists);
				echo $html = $smarty->fetch('tw_acg/widget/wap/list_json.tpl');
				break;
			case 'image':
				$smarty = new CSmarty();
				$smarty->assign('image', $lists);
				echo $html = $smarty->fetch('tw_acg/widget/wap/list_json.tpl');
				break;
			default:
