...
class content_tag {
	...
	public function lists($data) {
		$catid = intval($data['catid']);
		$field = $data['field'] ? : '*';
		if(!$this->set_modelid($catid) && !$data['modelid']) return false;
        if($data['modelid']) $this->db->set_model($data['modelid']); 
		if(isset($data['where'])) {
			$sql = $data['where'];
		} else {
	...
	public function types($data) {
        ...
        $limit = $data['limit'] ? : 10;
		if($typeid) {
			$sql = "select name,count(d.typeid) as total from acg_type a,acg_type_data d where module='content' AND a.typeid=d.typeid and a.typeid in ({$typeid}) group by d.typeid";
		} else {
			$sql = "select name,count(d.typeid) as total from acg_type a,acg_type_data d WHERE module='content' AND a.typeid=d.typeid group by d.typeid ORDER BY total desc limit {$limit}";
		}
		$this->db->query($sql);
		return $result = $this->db->fetch_array();
	...
	public function event($data) {
        ...
        $sql .= " AND `inputtime` BETWEEN {$begin} AND {$end}";
    ...
    public function calendar($data) {
        ...
        foreach($event as $val) {
            $key = (int)date('d', $val['inputtime']);
            $event_fmt[$key] = $val['id'];   
        }

        // 引入日历生成类
        pc_base::load_sys_class('calendar','',0);

        $cal = new Calendar($year, $month, $event_fmt);
        return $calstr = $cal->fetch();
