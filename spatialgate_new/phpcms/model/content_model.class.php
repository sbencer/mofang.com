...
class content_model extends model {
	...
	public function add_content($data,$isimport = 0) {
		...
		if($data['updatetime'] && !is_numeric($data['updatetime'])) {
			$systeminfo['updatetime'] = $inputinfo['system']['updatetime'] = strtotime($data['updatetime']);
		} elseif(!$data['updatetime']) {
			$systeminfo['updatetime'] = $inputinfo['system']['updatetime'] = SYS_TIME;
		} else {
			$systeminfo['updatetime'] = $inputinfo['system']['updatetime'] = $data['updatetime'];
		}
		$systeminfo['username'] = $inputinfo['system']['username'] = $data['username'] ? $data['username'] : param::get_cookie('admin_username');
		...
		if(isset($_POST['auto_thumb']) && $systeminfo['thumb'] == '' && isset($modelinfo['content'])) {
			...
			if(preg_match_all("/(src)=([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|bmp|png))\\2/i", $content, $matches)) {
				$systeminfo['thumb'] = $inputinfo['system']['thumb'] = $matches[3][$auto_thumb_no];
			}
		}
		//主表
	...
	public function edit_content($data,$id) {
		...
		if($data['islink']==1) {
			$systeminfo['url'] = $inputinfo['system']['url'] = $_POST['linkurl'];
		} else {
			//更新URL地址
			$urls = $this->url->show($id, 0, $systeminfo['catid'], $systeminfo['inputtime'], $data['prefix'],$inputinfo,'edit');
			$systeminfo['url'] = $inputinfo['system']['url'] = $urls[0];
		}
		//主表
	...
	private function search_api($id = 0, $data = array(), $action = 'update') {
        ...
        $search_data['id'] = $id;
        $search_data['modelid'] = $this->modelid;

		if($action == 'update') {
			$fulltext_array = getcache('model_field_'.$this->modelid,'model');
			foreach($fulltext_array AS $key=>$value){
                if($value['issearch'] == 1) {
                    $search_data[$key] = $data['system'][$key] ? $data['system'][$key] : $data['model'][$key];    
                }
                if($key == 'content'){
                    $search_data[$key] = strip_tags($data['model'][$key]);
                }
			}
			$this->search_db->update_search($this->modelid ,$id, $search_data);
		} elseif($action == 'delete') {
			$this->search_db->delete_search($this->modelid ,$id);
		}
	}
	/**
	 * 获取单篇信息
