define('mf/login_jp', ['jquery', 'jquery/cookie'], function(require, exports, module) {
    ...
    MFLogin = {
        ...
        urlBase: {
            loginURL: "http://u.mofang.jp/account/login", // 登陆send url
            regURL: "http://u.mofang.jp/account/register", // 注册send url
            logoutURL: "http://u.mofang.jp/account/logout", // 退出send url
            jumpeLoginURL: "", // 登陆后跳转的url
            jumpeRegURL: "", // 注册后跳转的url
            jumpeLogoutURL: "", // 退出后跳转的url
            vcodeURL: "http://u.mofang.jp/user/vcode"
        },
        // 错误提示
        ...
        changeVcode: function(id) {
            var srcLink =   'http://u.mofang.jp/captcha/captcha/v/53808a1e178ee.html?refresh=',
        ...
        regUserNameReg: function() { // 效验用户名
            ...
            if (!MFLogin.regStr.usReg.test(str_rp_cn)) {
                MFLogin.msgPrompt(_namePath.regErr, "正しい形式でユーザー名を入力してください");//用户名格式错误
        ...
        regMailReg: function(callback) { // 效验邮箱
            ...
            if (!MFLogin.regStr.mailReg.test(mailVal)) {
                MFLogin.msgPrompt(_namePath.regErr, "有効なメールアドレスを入力してください");//请正确填写Email地址
                //_namePath.userMailReg.focus();
                callback(null,flag);
            } else {
                $.ajax({
                    url:"http://u.mofang.jp/account/check_email" ,
                    type: "post",
                    data:"email" + '=' + mailVal,
                    dataType: "jsonp",
        ...
        regPassWordReg: function() { // 效验密码
            ...
            if (!MFLogin.regStr.pwReg.test(passwordVal)) {
                MFLogin.msgPrompt(_namePath.regErr, "パスワードが間違っています");//密码不正确
        ...
        regRepeatPassWordReg: function() { // 重复密码
            ...
            if (passwordVal.val() !== repeatPasswordVal) {
                MFLogin.msgPrompt(_namePath.regErr, "パスワードが一致していません");///两次密码不同
        ...
        regVerifyCodeReg: function() { // 效验证码
            ...
            if (!MFLogin.regStr.codeReg.test(_verifyCode)) {
                MFLogin.msgPrompt(_namePath.regErr, "正しい検証番号を入力してください");//验证码格式错误
        ...
        regAccountLogin: function() { // 效验用户名
            ...
            //     return true;
                ...
                MFLogin.msgPrompt(_namePath.loginErr, "ユーザー名を入力してください");
        ...
        regPassWordLogin: function() { // 效验登陆密码
            ...
            if (!MFLogin.regStr.pwReg.test(_passwordVal)) {
                MFLogin.msgPrompt(_namePath.loginErr, "パスワードを入力してください");
        ...
        regVerifyCodeLogin: function() { // 效验证码
            ...
            if (!MFLogin.regStr.codeReg.test(_verifyCode)) {
                MFLogin.msgPrompt(_namePath.loginErr, "正しい検証番号を入力してください");//验证码格式错误
        ...
        login: function() { // 登陆
            ...
            $.ajax({
                url:"http://u.mofang.jp/account/login",
