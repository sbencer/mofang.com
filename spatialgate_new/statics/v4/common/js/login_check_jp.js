define("login_jp/check", ["jquery", 'mf/login_jp', 'jquery/jq_cookie'], function(require, exports, module) {

    var $ = require("jquery");
    var MFLogin = require('mf/login_jp');
    ...
    // 页面打开时,异步更新登陆状态
    $.ajax({
        url: "http://u.mofang.jp/account/check_login",
        dataType: 'jsonp',
    ...
    if ($.cookie('mf_activity')) {
        if (!/account\/index/.test(window.location.href)) {
            ...
            window.location.href = 'http://u.mofang.jp/account/index';
        };
    }
    ...
    if ($.cookie('mf_websiteli')) {
        $.ajax({
            url: 'http://u.mofang.jp/account/ucenter_syslogin',
            dataType: 'jsonp',
            success: function(data) {
                if (data.data.sys) {
    ...
    if ($.cookie('mf_websitelo')) {
        $.ajax({
            url: 'http://u.mofang.jp/account/ucenter_syslogout',
            dataType: 'jsonp',
            success: function(data) {
                if (data.data.sys) {
                    $("body").append($(data.data.sys));
