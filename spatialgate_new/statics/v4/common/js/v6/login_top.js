...
define('login_top',['jquery','check_login'],function(require, exports, module) {
	...
	if(USE_LOCAL_DATA){
		getUserInfo = "/statics/v4/common/test/get_user.json"; //获取用户信息
		getUserLogout = "/statics/v4/common/test/get_user.json"; //用户退出接口
		getUserNotice = "/statics/v4/common/test/get_user_notice.json";
		ajaxMethod="json";
	}
	if(USE_TEST_DATA){
		    ...
		    success: function(res) {
		    	if(res && res.code==0){
		    		...
		    		if(res.data.sys_message.unread_count>0 || res.data.reply.unread_count>0 || res.data.recommend.unread_count>0){
		    			$(".login-end").find(".icon-red").show();
	    				$(".header .msg").find(".icon-red").show();
		    		}else{
		    			$(".login-end").find(".icon-red").hide();
