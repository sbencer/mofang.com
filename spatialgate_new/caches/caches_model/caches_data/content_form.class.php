...
class content_form {
	...
	function typeid($field, $value, $fieldinfo) {
		...
		switch($this->fields[$field]['boxtype']) {
			case 'radio':
				$string = form::radio($data, $value, "name='info[$field]' $fieldinfo[formattribute]", $setting['width'], $field);
				break;

			case 'checkbox':
				$string = form::checkbox($data, $value, "name='info[$field][]' $fieldinfo[formattribute]", 1, $setting['width'], $field);
				break;
		}
		return $string;
	}
