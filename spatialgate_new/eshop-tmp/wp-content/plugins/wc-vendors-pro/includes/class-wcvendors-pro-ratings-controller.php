...
class WCVendors_Pro_Ratings_Controller {
	...
	public function feedback_form()
	{
		...
		if ( ! isset( $_GET[ 'wcv_order_id' ] ) || ! is_user_logged_in() || ! isset( $_GET[ '_wpnonce' ] ) || ! wp_verify_nonce( $_GET[ '_wpnonce' ], 'wcv-leave_feedback' ) ) {
			echo sprintf( __( '<p>This page should not be accessed directly. Please return to the <a href="%s">my account page</a> and select an order to leave feedback. </p>', 'wcvendors-pro' ), get_permalink( woocommerce_get_page_id( 'myaccount' ) ) ); 
			return;
		}
	...
	public static function ratings_link( $vendor_id, $link = true, $link_text = '' ) { 
		...
		$url = apply_filters( 'wcv_ratings_link_url', WCVendors_Pro_Vendor_Controller::get_vendor_store_url( $vendor_id ) . '?ratings=all' ); 
	...
	// 	add_rewrite_rule( $permalink . '/([^/]*)/ratings/', 'index.php?post_type=product&vendor_shop=$matches[1]&ratings=all', 'top' );
