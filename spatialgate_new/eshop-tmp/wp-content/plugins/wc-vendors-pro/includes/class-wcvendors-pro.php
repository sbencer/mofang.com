...
class WCVendors_Pro {
	...
	public function __construct() {
		...
		$this->wcvendors_pro_store_controller = new WCVendors_Pro_Store_Controller( $this->get_plugin_name(), $this->get_version(), $this->get_debug() );
		...
		$this->wcvendors_pro_report_controller = new WCVendors_Pro_Reports_Controller( $this->wcvendors_pro, $this->version, $this->debug ); 
		$this->wcvendors_pro_vendor_controller = new WCVendors_Pro_Vendor_Controller( $this->wcvendors_pro, $this->version, $this->debug ); 
		$this->wcvendors_pro_product_form = new WCVendors_Pro_Product_Form( $this->wcvendors_pro, $this->version, $this->debug );  
		$this->wcvendors_pro_store_form = new WCVendors_Pro_Store_Form( $this->wcvendors_pro, $this->version, $this->debug );  
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-wcvendors-pro-store-controller.php';
	...
	private function define_public_hooks() {
		...
		// Vendor Controller 
		$this->loader->add_action( 'woocommerce_created_customer', 				$this->wcvendors_pro_vendor_controller, 	'apply_vendor_redirect', 11, 2 );
	...
	private function define_shared_hooks() {
		...
		if ( !$ratings_disabled ) { 
			...
			// $this->loader->add_filter( 'init', 					$this->wcvendors_pro_ratings_controller, 	'add_rewrite_rules' );
	...
	*/
	public function load_settings( $options ) {
		...
		$options[ ] = array(
			...
			'desc'     => __( 'This is the fee added on top of the amount.', 'wcvendors-pro' ),
			'id'       => 'commission_fee',
			'type'     => 'number',
		);
		...
		$options[ ] = array( 'name' => __( 'Product Add/Edit Form', 'wcvendors-pro' ), 'type' => 'title', 'desc' => __( 'Configure what to hide from all vendors when adding a product on the front end.', 'wcvendors-pro' ) );
