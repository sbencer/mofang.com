...
class WCVendors_Pro_Dashboard {
	...
	public function get_dashboard_quick_links(){ 
		...
		if ( ! $products_disabled ) $quick_links['product'] 		= array( 'url' => apply_filters( 'wcv_add_product_url', self::get_dashboard_page_url( 'product/edit' ) ),	 'label' => __( 'Add Product', 'wcvendors_pro' )  );
		if ( ! $coupons_disabled ) 	$quick_links['shop_coupon'] 	= array( 'url' => self::get_dashboard_page_url( 'shop_coupon/edit' ), 	'label' => __( 'Add Coupon', 'wcvendors_pro' )  ); 
	...
	{ 
		...
		switch ( $object ) {
			// Product permissions 
			case 'product':
				return ( WCV_Vendors::get_vendor_from_product( $post_id ) != get_current_user_id() ) ? false : true; 
				break;
			case 'shop_coupon':
