...
class WCVendors_Pro_Vendor_Controller {
	...
	public static function get_orders( $vendor_id, $date_range = null ) {
		...
		$sql = "
			SELECT id, order_id, product_id, vendor_id, total_shipping, total_due, qty, tax, status, time
	...
	public static function get_orders2( $vendor_id, $date_range = null, $reports = true ) {
		...
		if ( $orders ) { 
			...
			foreach ( $orders as $order ) { 
				...
				$wcv_order->order_id 			= $order->order_id; 
				$wcv_order->product_id			= $order->product_id;
				$wcv_order->qty					= $order->qty;
				$wcv_order->total_due			= $order->total_due;
				$wcv_order->total_tax			= $order->tax;
				$wcv_order->total_shipping		= $order->total_shipping;
				$wcv_order->status				= $order->status;
				$wcv_order->recorded_time		= date('Y-m-d', strtotime( $order->time ) );
				...
				foreach ( $order_items as $key => $order_item ) {
					if ( $order_item ['product_id' ] == $order->product_id || $order_item[ 'variation_id' ] == $order->product_id ) { 
						...
						$order_item[ 'commission_total' ] 	= $order->total_due;
				...
				$wcv_order->total 				= $wcv_order->product_total + $order->total_shipping + $order->tax; 
				$wcv_order->commission_total 	= $order->total_due + $order->total_shipping + $order->tax; 
	...
	public static function save_pending_vendor( $vendor_id ) { 
		...
		remove_action( 'woocommerce_created_customer', array( 'WCV_Vendor_Signup', 'save_pending' ), 10, 2 );
