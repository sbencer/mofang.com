...
class WCVendors_Pro_Commission_Controller {
	...
	public function process_commission(  $commission, $product_id, $product_price, $order, $qty ) { 
		...
		$vendor_id 					= get_post_field( 'post_author', $product_id );
	 	...
	 	// Adjust the product price if a coupon is applied and valid for the product and order 
		 	...
		 	if ( !empty ($coupons ) ) { 
				...
				foreach ( $coupons as $coupon ) {
					...
					if ( $coupon_owner == $product_owner ) { 
						$product_price = $product_price - $coupon[ 'discount_amount' ]; 
					} elseif ( in_array( 'administrator', (array) $coupon_user->roles ) || in_array( 'shop_manager', (array) $coupon_user->roles ) ) { 
						// If the store manager makes a coupon then still apply it. 
						$product_price = $product_price - $coupon[ 'discount_amount' ]; 
					} 
