				...
				if ( $cart_item['quantity'] > 0 ) {
					...
					if ( $cart_item['data']->needs_shipping() ) {
						...
						if ( $cart_item[ 'variation_id'] ) {	
							$rate = self::get_shipping_rate( $cart_item[ 'variation_id'], $cart_item['data']->post->post_author, $package, $settings ); 	
						} 					
	...
	public static function get_shipping_rate( $product_id, $vendor_id, $package, $settings ) {
		...
		if ( $shipping_type == 'flat' ) {
			...
			if ( $customer_country == $store_country ) { 
				...
				if ( ( array_key_exists('international_disable', $product_rates ) && 'yes' == $product_rates['national_disable'] ) || ( array_key_exists('international_disable', $store_rates ) && 'yes' ==  $store_rates['national_disable'] ) ) { 
				...
				if ( ( array_key_exists('international_disable', $product_rates ) && 'yes' == $product_rates['international_disable'] ) || ( array_key_exists('international_disable', $store_rates ) &&  'yes' ==  $store_rates['international_disable'] ) ) { 
					return $shipping_rate = false;
				}
			...
			if ( is_array( $product_shipping_table ) ) {  
				...
				foreach ( $product_shipping_table as $rate ) {
					...
					if ( $customer_country == strtolower( $rate[ 'country' ] ) && $customer_state == strtolower( $rate[ 'state' ] ) ) { 
						$shipping_rate->fee = $rate[ 'fee' ]; 
						return $shipping_rate; 
					} elseif ( $customer_country == $rate[ 'country' ] && $rate[ 'state' ] == '' ) { 
						$shipping_rate->fee = $rate[ 'fee' ]; 
						return $shipping_rate; 
					}
				}
			...
			if ( is_array( $store_shipping_table ) ){ 
				...
				foreach ( $store_shipping_table as $rate ) {
					...
					if ( strtolower( $customer_country ) == strtolower( $rate[ 'country' ] ) && strtolower( $customer_state ) == strtolower( $rate[ 'state' ] ) ) { 
						$shipping_rate->fee = $rate[ 'fee' ]; 
						return $shipping_rate; 
					} elseif ( strtolower( $customer_country ) == strtolower( $rate[ 'country' ] ) && $rate[ 'state' ] == '' && strtolower( $customer_state ) != strtolower( $rate[ 'state' ] ) ) { 
						$shipping_rate->fee = $rate[ 'fee' ]; 
						return $shipping_rate;  
					}
				}
