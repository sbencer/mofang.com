	...
	jQuery(document).ready(function($){	
     ...
     $('.add_to_wishlist_button').click(function(){
        ...
        $.ajax({
			...
			$('a#'+id_new).attr('href','<?php echo get_permalink($nc_wishlist_settings['nc_wishlist_page']); ?>')
			.text('<?php echo 	 $nc_wishlist_settings['nc_wishlist_view_wishlist_text']; ?>').attr("id","");
        ...
        $.ajax({
			...
			if (result[0].response == 'removed') {
				...
				location.href="<?php get_permalink($nc_wishlist_settings['nc_wishlist_page']); ?>";
				}
